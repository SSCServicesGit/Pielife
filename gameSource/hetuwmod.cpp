...
int HetuwMod::delayReduction = 50;
...
bool HetuwMod::minitechEnabled = false;
...
doublePair HetuwMod::drawUIRectFollowCamera(
    ...
    panelPos.y = lastScreenViewCenter.y - (HetuwMod::viewHeight / 2)
                 + (offsetFromBottomCenter.y * guiScale);
...
float HetuwMod::UIScale(float value) {
...
void HetuwMod::Say(const char *text, int cipherNum) {
    std::string ciphered;
    ...
    char *encoded = new char[len * 2 + 1];
    encodeDigits(finalMsg.c_str(), encoded);

    if (livingLifePage != NULL) {
        livingLifePage->hetuwSay(encoded);
    }
...
// Positive:1747 – Mean Pit Bull, 1642 – Semi-tame Wolf# pregnant, 1630 – Semi-tame Wolf, 1339 – Domestic Boar, 1341 – Domestic Boar with Piglet, 628 – Grizzly Bear, 632 – Shot Grizzly Bear#1, 635 – Shot Grizzly Bear#2,  631 – Hungry Grizzly Bear

// Negative:  418 – Wolf, 420 – Shot Wolf, 1323 – Wild Boar, 4762 – Sleepy Grizzly Bear, 764 – Rattle Snake, 1328 – Wild Boar with Piglet
...
void HetuwMod::drawHomeCords() {
	...
	doublePair drawPosA = lastScreenViewCenter;
	drawPosA.x -= HetuwMod::viewWidth/2 - (15*guiScale);
...
//     // anchor to the player, then push up by a fixed *screen* distance
//     playerNamePos.x = player->currentPos.x * CELL_D;
//     playerNamePos.y = player->currentPos.y * CELL_D;
//     playerNamePos.y += 34 * guiScale;
...
//         // Temporarily multiply the custom font by guiScale to cancel zoom
//         double saved = customFont->hetuwGetScaleFactor();
//         customFont->hetuwSetScaleFactor( saved * guiScale );
...
//         customFont->hetuwSetScaleFactor( saved );
//     }
//     else if ( iDrawNames == 2 ) {
//         float textWidth = livingLifePage->hetuwMeasureScaledHandwritingFont( player->name, guiScale );
//         drawRect( playerNamePos, textWidth / 2 + 6 * guiScale, 16 * guiScale );
//         setDrawColor( playerNameColor[0], playerNameColor[1], playerNameColor[2], 1 );
...
//         char playerName[48];
//         removeLastName( playerName, player->name );
//         float textWidth = livingLifePage->hetuwMeasureScaledHandwritingFont( playerName, guiScale );
//         drawRect( playerNamePos, textWidth / 2 + 6 * guiScale, 16 * guiScale );
//         setDrawColor( playerNameColor[0], playerNameColor[1], playerNameColor[2], 1 );
//         livingLifePage->hetuwDrawScaledHandwritingFont( playerName, playerNamePos, guiScale, alignCenter );
//     }
// }
...
void HetuwMod::drawAge() {
    ...
    drawPos.x = lastScreenViewCenter.x - 440 * guiScale;
...
void HetuwMod::drawTemp() {
    ...
    drawPos.x = lastScreenViewCenter.x - 660 * guiScale;
...
void HetuwMod::drawHunger() {
    ...
    if (bonus > 0) {
        snprintf(sBuf, sizeof(sBuf), "%d + %d / %d (%.2f)",
                 foodstore, bonus, foodcap, countdown);
    } else {
        snprintf(sBuf, sizeof(sBuf), "%d / %d (%.2f)",
                 foodstore, foodcap, countdown);
    }
} else {
    if (bonus > 0) {
        snprintf(sBuf, sizeof(sBuf), "%d + %d / %d",
                 foodstore, bonus, foodcap);
    } else {
    ...
    livingLifePage->hetuwDrawScaledHandwritingFont(
        sBuf, drawPos, guiScale * 0.9, alignCenter
    );
}
...
void HetuwMod::drawSpeed() {
    ...
    snprintf(sBuf, sizeof(sBuf), "MPS: %d.%02d", mpsWhole, mpsDecimal);
...
void HetuwMod::drawCurseToken() {
    ...
    if( ourLiveObject->excessCursePoints > 0 ) {
        snprintf(sBuf, sizeof(sBuf), "D.T");
        setDrawColor(1, 0, 0, 1.0);
    }
    else if( ourLiveObject->curseTokenCount > 0 ){
        snprintf(sBuf, sizeof(sBuf), "CXT%d", ourLiveObject->curseTokenCount);
        setDrawColor(1, 1, 1, 1.0);
    }
	else{
        snprintf(sBuf, sizeof(sBuf), "CXT0");
        setDrawColor(0, 0, 0, 0.0);
	}

    drawPos.x = lastScreenViewCenter.x - 760 * HetuwMod::guiScale;
    drawPos.y = lastScreenViewCenter.y - (HetuwMod::viewHeight / 2)
                + 25 * HetuwMod::guiScale;
...
void HetuwMod::drawHelp() {
	...
	drawPos.y -= lineHeight;
	// Pielife Addition
	if (bDrawHostilePlayers) setHelpColorSpecial();
	else setHelpColorNormal();
