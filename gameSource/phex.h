...
// simple struct to remember explored tiles
struct MapTile {
    int objID = -1;   // ground object id
};

// --- inside class HetuwMod ---
public:
    std::unordered_map<long long, MapTile> exploredMap;

    void recordTile(int x, int y);
	...
	//Pielife
	static void chatCmdSAYRED(std::vector<std::string> input);
	static void chatCmdSAYGREEN(std::vector<std::string> input);
	static void chatCmdSAYLIME(std::vector<std::string> input);
	static void chatCmdSAYRGB(std::vector<std::string> input);
