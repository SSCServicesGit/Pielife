...
void HetuwMod::drawNavigation() {
    ...
    for (int i = 0; i < gameObjects->size(); i++) {
        ...
        if (ourLiveObject->killMode) {
            weAreAttacking = true;
        }

        if (ourEmot == 3066 && theirEmot == 3815) {
            theyAreKillable = true; 
        }
        if (theirEmot == 3066 && ourEmot == 3815) {
            weAreKillable = true;   
        }
    }
...
void HetuwMod::drawCurseToken() {
    ...
    if( ourLiveObject->excessCursePoints > 0 ) {
        ...
        snprintf(sBuf, sizeof(sBuf), "CX %d", ourLiveObject->curseTokenCount);
        setDrawColor(1, 0, 1, 1.0);
    }
	else{
        snprintf(sBuf, sizeof(sBuf), "CX 0");
        setDrawColor(1, 0.5, 0, 1.0);
	}
